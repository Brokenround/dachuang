...
class DEEExample(object):
    def __init__(self, annguid, detail_align_dict, only_inference=False):
        ...
        if only_inference:
            ...
            # 记录（recguid，event_name，event_dict）的列表
...
class DEEExampleLoader(object):
    def __init__(self, rearrange_sent_flag, max_sent_len, only_inference=False):
        """
        DEEExampleLoader 类的初始化函数。

        参数:
        - rearrange_sent_flag: 布尔值，表示是否重新排列句子。
        - max_sent_len: 整数，表示句子的最大长度。
        - only_inference: 布尔值，表示是否仅进行推断。
        """
    ...
    def __call__(self, dataset_json_path, sentence_input_dict=None):
        """
       类的调用函数，用于处理数据集或进行模型推断。

       参数:
       - dataset_json_path: 数据集的 JSON 文件路径。
       - sentence_input_dict: 包含句子信息的字典，用于模型推断。
...
class DEEFeature(object):
    def __init__(self, guid, ex_idx, doc_token_id_mat, doc_token_mask_mat, doc_token_label_mat,
        ...
        - guid: 唯一标识符。
        - ex_idx: 示例行索引，用于回溯。
        - doc_token_id_mat: 文档中的token id矩阵。
        - doc_token_mask_mat: 文档中的token掩码矩阵。
        - doc_token_label_mat: 文档中的token标签矩阵。
        - span_token_ids_list: span的token id列表。
        - span_dranges_list: span的字符范围列表。
        - event_type_labels: 事件类型标签列表。
        - event_arg_idxs_objs_list: 事件参数索引对象列表。
        - valid_sent_num: 有效的句子数量。

        属性:
        - guid: 唯一标识符。
        - ex_idx: 示例行索引。
        - valid_sent_num: 有效的句子数量。
        - doc_token_ids: 文档中的token id张量。
        - doc_token_masks: 文档中的token掩码张量。
        - doc_token_labels: 文档中的token标签张量。
        - span_token_ids_list: span的token id列表。
        - span_dranges_list: span的字符范围列表。
        - event_type_labels: 事件类型标签列表。
        - event_arg_idxs_objs_list: 事件参数索引对象列表。
        - event_idx2field_idx2pre_path2cur_span_idx_set: 事件索引到字段索引到前路径到当前span索引集合的映射。

        方法:
        - generate_dag_info_for: 为预测的span生成DAG信息。
        - get_event_args_objs_list: 获取事件参数对象列表。
        - build_key_event_sent_info: 构建关键事件句子信息。
        - build_dag_info: 构建DAG信息。
        - is_multi_event: 判断是否为多事件。
        """
        self.guid = guid
    ...
    def generate_dag_info_for(self, pred_span_token_tup_list, return_miss=False):
        ...
        参数:
        - pred_span_token_tup_list: 预测的span的token tuple列表。
        - return_miss: 布尔值，表示是否返回丢失的信息。

        返回:
        - pred_dag_info: 预测的DAG信息。
        - missed_span_idx_list: 丢失的span索引列表。
        - missed_sent_idx_list: 丢失的句子索引列表。
        """
    ...
    def build_key_event_sent_info(self):
        ...
        返回:
        - event_idx2key_sent_idx_set: 事件索引到关键句子索引集合的映射。
        - doc_sent_labels: 文档中的句子标签列表。
        """
...
class DEEFeatureConverter(object):
    def __init__(self, entity_label_list, event_type_fields_pairs,
        ...
        """
        DEEFeatureConverter 类的初始化函数。

        参数:
        - entity_label_list: 实体标签列表。
        - event_type_fields_pairs: 事件类型和事件字段的对应关系列表。
        - max_sent_len: 最大句子长度。
        - max_sent_num: 最大句子数量。
        - tokenizer: 分词器。
        - ner_fea_converter: NERFeatureConverter 类的实例。
        - include_cls: 是否包含CLS token。
        - include_sep: 是否包含SEP token。
        ...
        - max_sent_num: 最大句子数量。
        - tokenizer: 分词器。
        - truncate_doc_count: 记录因为超过最大句子数量而被截断的文档数量。
        - truncate_span_count: 记录因为超过最大句子长度而被截断的span数量。
        - ner_fea_converter: NERFeatureConverter 类的实例。
        - include_cls: 是否包含CLS token。
        - include_sep: 是否包含SEP token。
        - entity_label2index: 实体标签到索引的映射。
        - event_type2index: 事件类型到索引的映射。
        - event_type_list: 事件类型列表。
        - event_fields_list: 事件字段列表。

        方法:
        - convert_example_to_feature: 将示例转换为特征。
        - __call__: 将示例列表转换为适用于文档级事件抽取的特征。
        """
    ...
    def __call__(self, dee_examples, log_example_num=0):
        """
        将示例列表转换为适用于文档级事件抽取的特征。

        参数:
        - dee_examples: DEEExample 类的实例列表。
        - log_example_num: 记录日志的示例数量。
...
def convert_dee_features_to_dataset(dee_features):
    """
    将 DEEFeature 实例列表转换为数据集。

    参数:
    - dee_features: DEEFeature 实例列表。
...
def measure_dee_prediction(event_type_fields_pairs, features, event_decode_results,
    ...
    参数:
    - event_type_fields_pairs: 事件类型和事件字段的对应关系列表。
    - features: DEEFeature 实例列表。
    - event_decode_results: DEE 预测结果。
    - dump_json_path: 结果保存路径。
    - with_post_process: 是否进行后处理。
    - vocab_list: 词汇表列表。
...
def measure_dee_prediction_with_pickle_input(event_type_fields_pairs, pickle_data, event_decode_results,
                                             dump_json_path=None, with_post_process=False, vocab_list=None):
    """
    使用 pickle 数据输入测量 DEE 预测结果的性能。

    参数:
    - event_type_fields_pairs: 事件类型和事件字段的对应关系列表。
    - pickle_data: 包含金标和其他信息的 pickle 数据。
    - event_decode_results: DEE 预测结果。
    - dump_json_path: 结果保存路径。
    - with_post_process: 是否进行后处理。
    - vocab_list: 词汇表列表。
...
def aggregate_task_eval_info(eval_dir_path, target_file_pre='dee_eval', target_file_suffix='.json',
    ...
    """
    聚合任务评估信息，收集所有保存的评估结果。

    参数:
    - eval_dir_path: 评估结果文件夹路径。
    - target_file_pre: 目标文件名前缀。
    - target_file_suffix: 目标文件名后缀。
    - dump_name: 聚合结果保存文件名。
    - dump_flag: 是否保存聚合结果。
...
def print_total_eval_info(data_span_type2model_str2epoch_res_list,
                          ...
                          target_set='test'):
    """
    打印最终性能，选择在 dev 集上的最佳 epoch 并输出在 test 集上的性能。

    参数:
    - data_span_type2model_str2epoch_res_list: 聚合的评估信息。
    - metric_type: 性能度量类型（'micro' 或 'macro'）。
    - span_type: 预测的跨度类型。
    - model_strs: 模型类型字符串列表。
    - target_set: 目标集（'dev' 或 'test'）。
...
def print_single_vs_multi_performance(mstr_bepoch_list, base_dir, features,
    ...
    参数:
    - mstr_bepoch_list: 包含模型类型和最佳 epoch 的列表。
    - base_dir: 存储结果的基础目录。
    - features: 特征数据。
    - metric_type: 性能度量类型（'micro' 或 'macro'）。
    - data_type: 数据类型。
...
def merge_multi_events(event_idx2obj_idx2field_idx2token_tup, vocab_list, delimiter='/', process_for_display=False):
    """
    合并多个事件，如果它们除一个角色外的所有角色都相同。如果不同的角色实体具有包含关系，则保留较长的那个。

    参数:
    - event_idx2obj_idx2field_idx2token_tup: 包含多个事件的数据结构。
    - vocab_list: 词汇表列表。
    - delimiter: 合并实体时使用的分隔符。
    - process_for_display: 是否为显示模式。
...
def aggregate_multi_entities(entity_id_list, vocab_list, delimiter='/', unk_tok='[UNK]'):
    """
    将多个实体聚合成一个。规则如下：
    1. 如果一个实体完全被另一个实体重叠，将其移除。
    2. 使用 '/' 连接实体。

    参数:
    - entity_id_list: 包含多个实体 ID 的列表。
    - vocab_list: 词汇表列表。
    - delimiter: 连接实体时使用的分隔符。
    - unk_tok: 未知令牌的标识符。
